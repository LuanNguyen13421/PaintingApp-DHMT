...
namespace _19120285_BT3
{
    internal class EquilateralTriangle : Shape
    {
        ...
        public EquilateralTriangle(Point _pStart, Point _pEnd)
        {
            base.SetPoint(_pStart, _pEnd);
            // Tìm tâm của hình chữ nhật tạo ra từ 2 điểm đầu vào, đó là trung điểm của đường cao
            int centerX = (int)Math.Round((_pStart.X + _pEnd.X) * 1.0 / 2);
            int centerY = (int)Math.Round((_pStart.Y + _pEnd.Y) * 1.0 / 2);
            // Tìm độ dài ngắn hơn giữa 2 cạnh kề của hình chữ nhật
            // Nếu độ dài đó là của đường song song với Oy thì là đường cao của tam giác đều
            // Nếu độ dài đó là của đường song song với Ox thì là cạnh
            ...
            {
                edge = dx;
                length = (int)Math.Round(edge * Math.Sqrt(3) / 2);
            }
            else
            {
                length = dy;
                edge = (int)Math.Round(length * 2 / Math.Sqrt(3));
            }
            // Gán 3 đỉnh của hình tam giác đều khi đã biết trung điểm đường cao, độ dài đường cao và độ dài cạnh
            topPoint.X = centerX;
            topPoint.Y = (int)Math.Round(centerY - length * 1.0 / 2);

            bottomLeftPoint.X = (int)Math.Round(centerX - edge * 1.0 / 2);
            bottomLeftPoint.Y = topPoint.Y + length;

            bottomRightPoint.X = (int)Math.Round(centerX + edge * 1.0 / 2);
            bottomRightPoint.Y = topPoint.Y + length;
        }
        ...
        public override void SetControlPoint()
        {
            ...
            max_x = bottomRightPoint.X;
            max_y = bottomRightPoint.Y;
            min_x = bottomLeftPoint.X;
            min_y = topPoint.Y;
            mid_x = (max_x + min_x) / 2;
            mid_y = (max_y + min_y) / 2;
        ...
        {
            // Vẽ 3 đoạn thẳng nối 3 đỉnh
            Shape line1 = new Line(topPoint, bottomRightPoint);
            Shape line2 = new Line(bottomRightPoint, bottomLeftPoint);
            Shape line3 = new Line(bottomLeftPoint, topPoint);
